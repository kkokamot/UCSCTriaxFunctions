function [pf1, pf2, pv1, pv2, LVDT1, LVDT2] = plot_onboardLVDTS(exp_num, title_text, fig_num, type, hold_time_string, col_value)
   ...
   %end
    ...
    for i = [indexes]
        ...
        LVDT1 = (file_df.LVDT1(file_df.OG_Index == last_index)-file_df.LVDT1(first_index))*10^3;
        LVDT2 = (file_df.LVDT2(file_df.OG_Index == last_index)-file_df.LVDT2(first_index))*10^3;
        %if (LVDT1+LVDT2)/2 > 1/2*LVDT1 && (LVDT1 +LVDT2)/2 > 1/2*LVDT2
        %    LVDT_ave = ((file_df.LVDT1(hold_indices)-file_df.LVDT1(first_index)) + (file_df.LVDT2(hold_indices)-file_df.LVDT2(first_index)))/2;
        %else
            if LVDT1 > LVDT2
                LVDT_ave = file_df.LVDT1(hold_indices)-file_df.LVDT1(first_index);
            elseif LVDT2 > LVDT1
                LVDT_ave = file_df.LVDT2(hold_indices)-file_df.LVDT2(first_index);
            elseif LVDT2 == LVDT1
                LVDT_ave = nan;
            end
        %end
        figure(fig_num)
        if type == "shear"
            plot(LVDT_ave*10^3, file_df.shear(hold_indices)-file_df.shear(first_index), 'Color', col_value)
            hold on
            %plot(LVDT1, fit1, '-','Color', col_value)
            %text(LVDT1(end), fit1(end), string(pf1(1)/10^3))
            %plot(file_df.LoadingPlattenDispHighGain(reload_indices) -file_df.LoadingPlattenDispHighGain(first_index) + LVDT1(end), file_df.shear(reload_indices)-file_df.shear(first_index), '--', 'Color', col_value)
            ...
            %plot(file_df.LoadingPlattenDispHighGain(reload_indices) -file_df.LoadingPlattenDispHighGain(first_index) + LVDT2(end), file_df.shear(reload_indices)-file_df.shear(first_index), '--', 'Color', col_value)
            %subplot(1,3,3)
