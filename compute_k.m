function compute_k(exp_num, eff_normal)
    close all 
    ...
    if check_for_previous == 1
        ...
        if check == 'N'
            ...
            open("UC" + exp_num + "_stiffness.fig")
            return
        end
    end
    ...
    figure(1)
    plot(loadup_disp, loadup_friction)
    [cut_disp, ~] = ginput(2);

    l_fit_disp = loadup_disp(loadup_disp > cut_disp(1) & loadup_disp < cut_disp(2));
    l_fit_friction = loadup_friction(loadup_disp > cut_disp(1) & loadup_disp < cut_disp(2));
    pf = polyfit(l_fit_disp,l_fit_friction,1);
    k_loadup_fit = polyval(pf,l_fit_disp);
    k_loadup = pf(1)/1000*eff_normal;
    k_l_start_index = file_df.OG_Index(file_df.LoadingPlattenDispHighGain == l_fit_disp(1));
    k_l_end_index = file_df.OG_Index(file_df.LoadingPlattenDispHighGain == l_fit_disp(end));
    ...
    for index = [1:length(heal_index)]
        ...
        start_cut = 10;
        length_cut = round(length(snip_disp)/4);
        fit_disp = snip_disp(start_cut: start_cut + length_cut);
        fit_fric = snip_friction(start_cut: start_cut + length_cut);
        pf = polyfit(fit_disp, fit_fric, 1);
        k_reload(index) = pf(1)/1000*eff_normal; %1/mm*mm/micron*MPa --> MPa/um
        ...
        k_r_end_index(index) = heal_index(index) + start_cut + length_cut;
    end
